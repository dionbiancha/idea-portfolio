...
interface Props {
  ...
  sections: { about: string; projects: string; skills: string; contact: string };
}
      ...
      <Menu>
        {({ open }) => (
          <>
            ...
            <Overlay contentRef={menu} open={open} animationFinish={animationFinish} classOnOpen="w-full" classOnClose="w-0" className="text-green-600"/>
                      ...
                      {({ active }: { active: boolean }) => (
                        ...
                        >
                          <MenuItem active={active}>
                            {language.value === 'en' ? 'Contact' : 'Contato'}
                          </MenuItem>
                        </Animated.FromDirection>
                      )}
                    </Menu.Item>
