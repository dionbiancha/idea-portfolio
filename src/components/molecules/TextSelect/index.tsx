...
export default function TextSelect({ options, state, className }: TextSelectProps) {
  ...
  return (
    <Listbox as="div" className={className} value={selectedOption} onChange={setSelectedOption}>
      {({ open }) => (
        <>
          <Listbox.Button className="flex items-center px-4 py-2 text-gray-700 bg-white rounded-lg focus:outline-none focus:ring-2 ring-green-500">
          ...
          >
            <Listbox.Options
              static
              className="overflow-hidden text-3xl bg-white shadow-md rounded-xl focus:outline-none focus:ring-2 ring-green-500 sm:text-base"
            >
              {options.map(option => (
