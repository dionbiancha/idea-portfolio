...
export default function BlogPost({ data, className }: BlogsProps) {
  ...
  return (
    <article className={classNames(className, 'focus-within:ring-2 ring-green-500 ring-offset-8 rounded-lg')}>
      <a href={url} className="relative inline-block rounded-lg group focus:outline-none">
        <ContentTitle>{title}</ContentTitle>
        <svg className="-left-1 bottom-0.5 -z-10 absolute w-0 h-2 text-green-200 transition-all group-focus:w-full-2 group-hover:w-full-2" fill="currentColor">
